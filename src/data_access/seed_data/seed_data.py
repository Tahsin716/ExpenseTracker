...
class SeedData(DataAccess):
    ...
    def expense_seed(self):
        if not self.session.execute(select(func.count(Expense.expense_id))).scalar():
            expenses = [
                Expense(
                    category_id=1,
                    amount=300.0,
    ...
    def sale_item_seed(self):
        if not self.session.execute(select(func.count(SaleItem.sale_item_id))).scalar():
            sale_items = [
                SaleItem(
                    sale_id=1,
                    inventory_item_id=1,
    ...
    def sale_seed(self):
        if not self.session.execute(select(func.count(Sale.sale_id))).scalar():
            sales = [
                Sale(
    ...
    def user_seed(self):
        if not self.session.execute(select(func.count(User.user_id))).scalar():
            users = [
    ...
    def inventory_seed(self):
        if not self.session.execute(select(func.count(InventoryItem.item_id))).scalar():
            inventory_items = [
    ...
    def customer_seed(self):
        if not self.session.execute(select(func.count(Customer.customer_id))).scalar():
            customers = [
    ...
    def category_seed(self):
        if not self.session.execute(select(func.count(Category.category_id))).scalar():
            categories = [
